  ...
  useEffect(() => {
    async function fetchPendingReward() {
      try {
        ...
        // todo: do not assume [0] or that rewardToken has 18 decimals (only works w/ mastebreederv2 currently)
    ...
    if (
      ...
      farm &&
      library &&
      (farm.breeder === Breeder.MASTERBREEDER_V2 || farm.breeder === Breeder.MINIBREEDER)
    ) {
      fetchPendingReward()
    }
