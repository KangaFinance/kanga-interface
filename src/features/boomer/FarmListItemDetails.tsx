...
import { Breeder, PairType } from './enum'
import { Disclosure, Transition } from '@headlessui/react'
import React, { useState } from 'react'
import { usePendingKanga, useUserInfo } from './hooks'
...
import useMasterBreeder from './useMasterBreeder'
...
const FarmListItem = ({ farm }) => {
  ...
  const liquidityToken = new Token(
    ...
    farm.pair.type === PairType.TROOP ? Number(farm.pair.asset.decimals) : 18,
    farm.pair.symbol,
    farm.pair.name
  )
  ...
  const pendingKanga = usePendingKanga(farm)

  const reward = usePendingReward(farm)

  const APPROVAL_ADDRESSES = {
    [Breeder.MASTERBREEDER]: { [ChainId.MAINNET]: MASTERBREEDER_ADDRESS[ChainId.MAINNET] },
    [Breeder.MASTERBREEDER_V2]: { [ChainId.MAINNET]: MASTERBREEDER_V2_ADDRESS[ChainId.MAINNET] },
    [Breeder.MINIBREEDER]: {
      [ChainId.MATIC]: MINIBREEDER_ADDRESS[ChainId.MATIC],
      [ChainId.XDAI]: MINIBREEDER_ADDRESS[ChainId.XDAI],
      [ChainId.HARMONY]: MINIBREEDER_ADDRESS[ChainId.HARMONY],
      [ChainId.ARBITRUM]: MINIBREEDER_ADDRESS[ChainId.ARBITRUM],
    },
  }
  ...
  const [approvalState, approve] = useApproveCallback(typedDepositValue, APPROVAL_ADDRESSES[farm.breeder][chainId])

  const { deposit, withdraw, harvest } = useMasterBreeder(farm.breeder)
  ...
  return (
    ...
    >
      <Disclosure.Panel className="flex flex-col w-full border-t-0 rounded rounded-t-none bg-dark-800" static>
            ...
            {approvalState === ApprovalState.NOT_APPROVED || approvalState === ApprovalState.PENDING ? (
              ...
              <Button
                ...
                onClick={async () => {
                  setPendingTx(true)
                  try {
                    // KMP decimals depend on asset, KLP is always 18
        ...
        {pendingKanga && pendingKanga.greaterThan(ZERO) && (
          <div className="px-4 pb-4">
            ...
            >
              {i18n._(t`Harvest ${formatNumber(pendingKanga.toFixed(18))} KANGA ${
