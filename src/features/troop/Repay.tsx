...
import {
  Percent,
  KANGAFINANCE_MULTISWAPPER_ADDRESS,
  KANGAFINANCE_MULTI_EXACT_SWAPPER_ADDRESS,
  WNATIVE,
...
import { TroopMaker } from '../../entities'
...
const DEFAULT_troop_REPAY_SLIPPAGE_TOLERANCE = new Percent(5, 100)
...
export default function Repay({ pair }: RepayProps) {
  ...
  // State
  const [useMobRepay, setUseMobRepay] = useState<boolean>(pair.asset.mobBalance.gt(0))
  const [useMobRemove, setUseMobRemoveCollateral] = useState<boolean>(true)
  ...
  const balance = useMobRepay
    ? toAmount(pair.asset, pair.asset.mobBalance)
    : assetNative
    ? info?.ethBalance
    : pair.asset.balance
  ...
  const allowedSlippage = useUserSlippageToleranceWithDefault(DEFAULT_troop_REPAY_SLIPPAGE_TOLERANCE)
  ...
  const warnings = new Warnings()
    .addError(
      assetNative && !useMobRepay && pinRepayMax,
      `You cannot MAX repay ${pair.asset.tokenInfo.symbol} directly from your wallet. Please deposit your ${pair.asset.tokenInfo.symbol} into the Mob first, then repay. Because your debt is slowly accrueing interest we can't predict how much it will be once your transaction gets mined.`
    )
    .addError(
    ...
    .addError(
      ...
      new Warning(
        ...
        `Please make sure your ${useMobRepay ? 'Mob' : 'wallet'} balance is sufficient to repay and then try again.`,
        true
      )
    )
    .addError(
  ...
  async function onExecute(maker: TroopMaker) {
    ...
    if (swap && trade) {
      ...
      maker.removeCollateral(pair.userCollateralShare, true)
      maker.mobTransferCollateral(pair.userCollateralShare, KANGAFINANCE_MULTI_EXACT_SWAPPER_ADDRESS[chainId || 1])
      maker.repayShare(pair.userBorrowPart)
      ...
      maker.action(
        KANGAFINANCE_MULTI_EXACT_SWAPPER_ADDRESS[chainId || 1],
        ZERO,
      ...
      maker.repayPart(pair.userBorrowPart, true)

      if (!useMobRemove) {
        maker.mobWithdrawCollateral(ZERO, BigNumber.from(-1))
      }

      summary = 'Repay All'
    } else {
      if (pinRepayMax && pair.userBorrowPart.gt(0) && balance.gte(pair.currentUserBorrowAmount.value)) {
        maker.repayPart(pair.userBorrowPart, useMobRepay)
        summary = 'Repay Max'
      } else if (displayRepayValue.toBigNumber(pair.asset.tokenInfo.decimals).gt(0)) {
        maker.repay(displayRepayValue.toBigNumber(pair.asset.tokenInfo.decimals), useMobRepay)
        summary = 'Repay'
      }
      if (
        ...
        maker.removeCollateral(share, useMobRemove)
  ...
  return (
    <>
      ...
      <SmartNumberInput
        ...
        useMobTitle={`Repay ${pair.asset.tokenInfo.symbol} from`}
        useMob={useMobRepay}
        setUseMob={setUseMobRepay}
        maxTitle="Balance"
        max={balance}
        pinMax={pinRepayMax}
      ...
      <SmartNumberInput
        ...
        useMobTitle={`Remove ${pair.collateral.tokenInfo.symbol} to`}
        useMob={useMobRemove}
        setUseMob={setUseMobRemoveCollateral}
      ...
      <TroopApproveButton
        color="pink"
        content={(onMake: any) => (
          <TokenApproveButton value={displayRepayValue} token={assetToken} needed={!useMobRepay}>
            <Button onClick={() => onMake(pair, onExecute)} disabled={actionDisabled}>
              {actionName}
            </Button>
          </TokenApproveButton>
        )}
