...
import { Percent, KANGAFINANCE_MULTISWAPPER_ADDRESS, WNATIVE } from '@kangafinance/sdk'
...
export default function Borrow({ pair }: BorrowProps) {
  ...
  // State
  const [useMobCollateral, setUseMobCollateral] = useState<boolean>(pair.collateral.mobBalance.gt(0))
  const [useMobBorrow, setUseMobBorrow] = useState<boolean>(true)
  ...
  const collateralBalance = useMobCollateral
    ? pair.collateral.mobBalance
    : assetNative
    ? info?.ethBalance
  ...
  collateralWarnings.add(
    ...
    `Please make sure your ${useMobCollateral ? 'Mob' : 'wallet'} balance is sufficient to deposit and then try again.`,
    true
  )
  ...
  async function onExecute(maker: TroopMaker): Promise<string> {
    ...
    if (borrowValueSet) {
      if (displayUpdateOracle) {
        maker.updateExchangeRate(true, ZERO, ZERO)
      }

      if (swap && !useMobCollateral) {
        maker.mobDepositCollateral(collateralValue.toBigNumber(pair.collateral.tokenInfo.decimals))
      }

      maker.borrow(
        borrowValue.toBigNumber(pair.asset.tokenInfo.decimals),
        swap || useMobBorrow,
        swap ? KANGAFINANCE_MULTISWAPPER_ADDRESS[chainId || 1] : ''
      )
    }
    if (borrowValueSet && trade) {
      ...
      maker.action(
        KANGAFINANCE_MULTISWAPPER_ADDRESS[chainId || 1],
        ZERO,
  ...
  return (
    <>
      ...
      <SmartNumberInput
        ...
        useMobTitle={`Add ${pair.collateral.tokenInfo.symbol} collateral from`}
        useMob={useMobCollateral}
        setUseMob={setUseMobCollateral}
        maxTitle="Balance"
      ...
      <SmartNumberInput
        ...
        useMobTitle={`Borrow ${pair.asset.tokenInfo.symbol} to`}
        useMob={useMobBorrow}
        setUseMob={setUseMobBorrow}
        maxTitle="Max"
      ...
      <TroopApproveButton
        color="pink"
        content={(onMake: any) => (
          <TokenApproveButton value={collateralValue} token={collateralToken} needed={!useMobCollateral}>
