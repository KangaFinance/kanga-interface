...
import { setFromMobBalance } from '../../../state/limit-order/actions'
...
const BalancePanel: FC = () => {
  ...
  const { walletBalances, mobBalances, currencies } = useDerivedLimitOrderInfo()
  ...
  const handleMaxInput = useCallback(
    (mob) => {
      if (mob) {
        ...
        dispatch(setFromMobBalance(false))
      }
    },
    [mobBalances, dispatch, maxAmountInput, onUserInput]
  )
    ...
    <div className="grid grid-cols-2 bg-dark-700 rounded-b px-5 py-1">
      <div className="flex gap-2">
        ...
        <Typography variant="sm" className="text-secondary" onClick={() => handleMaxInput(true)}>
          {mobBalances[Field.INPUT]?.toSignificant(6, { groupSeparator: ',' })} {currencies[Field.INPUT]?.symbol}
        </Typography>
      </div>
      <div className="flex gap-2">
        <Typography variant="sm" weight={700}>
