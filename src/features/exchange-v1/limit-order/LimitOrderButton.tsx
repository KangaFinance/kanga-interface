...
import { KANGAMOB_ADDRESS, ChainId, Currency } from '@kangafinance/sdk'
import Button, { ButtonProps } from '../../../components/Button'
import { Field, setFromMobBalance } from '../../../state/limit-order/actions'
import React, { FC, useCallback, useState } from 'react'
import { useAddPopup, useWalletModalToggle } from '../../../state/application/hooks'
import { useDerivedLimitOrderInfo, useLimitOrderState } from '../../../state/limit-order/hooks'
import useLimitOrderApproveCallback, { MobApprovalState } from '../../../hooks/useLimitOrderApproveCallback'

import Alert from '../../../components/Alert'
import { AppDispatch } from '../../../state'
import ConfirmLimitOrderModal from './ConfirmLimitOrderModal'
import Dots from '../../../components/Dots'
import { LimitOrder } from '@kangafinance/sdk'
...
const LimitOrderButton: FC<LimitOrderButtonProps> = ({ currency, color, ...rest }) => {
  ...
  const { fromMobBalance, orderExpiration, recipient } = useLimitOrderState()
  ...
  const showLimitApprove =
    (approvalState === MobApprovalState.NOT_APPROVED || approvalState === MobApprovalState.PENDING) && !permit
  ...
  const disabled =
    !!inputError ||
    approvalState === MobApprovalState.PENDING ||
    depositPending ||
  ...
  const deposit = useCallback(async () => {
    ...
    dispatch(setFromMobBalance(true))
  ...
  if (depositPending)
    button = (
      <Button disabled={disabled} color={disabled ? 'gray' : color} onClick={deposit} {...rest}>
        <Dots>{i18n._(t`Depositing ${currency.symbol} into Mob`)}</Dots>
      </Button>
    )
  else if (!account)
    button = (
  ...
  else if ((permit && !fromMobBalance) || (!permit && approvalState === MobApprovalState.APPROVED && !fromMobBalance))
    button = (
      <Button disabled={disabled} color={disabled ? 'gray' : 'blue'} onClick={deposit} {...rest}>
        {i18n._(t`Deposit ${currency.symbol} into Mob`)}
      </Button>
    )
