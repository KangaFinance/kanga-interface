import { useBlock, useDayData, useKangaPairs } from '../../../../services/graph'
...
export default function PairChartCard(props: PairChartCardProps): JSX.Element {
  ...
  const pair = useKangaPairs({ subset: [props.pair], shouldFetch: !!props.pair, chainId })?.[0]
  const pair1d = useKangaPairs({
    subset: [props.pair],
    block: block1d,
    shouldFetch: !!props.pair && !!block1d,
    chainId,
  })?.[0]
  const pair2d = useKangaPairs({
    subset: [props.pair],
    block: block2d,
