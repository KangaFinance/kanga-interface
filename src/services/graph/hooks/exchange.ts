...
export function useKangaPrice(variables = undefined, swrConfig: SWRConfiguration = undefined) {
  const { data } = useSWR(['kangaPrice', JSON.stringify(variables)], () => getKangaPrice(variables), swrConfig)
  return data
}
...
export function useKangaPairs(
  { timestamp, block, chainId, shouldFetch = true, user, subset }: useKangaPairsProps,
  swrConfig: SWRConfiguration = undefined
) {
  const blockFetched = useBlock({ timestamp, chainId, shouldFetch: shouldFetch && !!timestamp })
  block = block ?? (timestamp ? blockFetched : undefined)
  ...
  const { data } = useSWR(
    shouldFetch ? ['kangaPairs', chainId, JSON.stringify(variables)] : null,
