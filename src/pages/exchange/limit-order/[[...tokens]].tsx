import { ChainId, Token } from '@kangafinance/sdk'
...
import limitOrderPairList from '@kangafinance/limit-order-pair-list/dist/limit-order.pairlist.json'
...
function LimitOrder() {
  ...
  // Limit order state
  const { independentField, typedValue, recipient, limitPrice, fromMobBalance } = useLimitOrderState()

  // Limit order derived state
  const { currencies, parsedAmounts, walletBalances, mobBalances, currentPrice } = useDerivedLimitOrderInfo()
  ...
  const atMaxAmountInput = Boolean(
    fromMobBalance
      ? parsedAmounts[Field.INPUT]?.equalTo(mobBalances[Field.INPUT])
  ...
  const handleMaxInput = useCallback(() => {
    if (fromMobBalance) {
      onUserInput(Field.INPUT, mobBalances[Field.INPUT].toExact())
    } else {
      maxAmountInput && onUserInput(Field.INPUT, maxAmountInput.toExact())
    }
  }, [mobBalances, fromMobBalance, maxAmountInput, onUserInput])
  ...
  return (
    <Container id="limit-order-page" className="py-4 space-y-6 md:py-8 lg:py-12" maxWidth="2xl">
      <Head>
        <title>{i18n._(t`Limit order`)} | Kanga</title>
        <meta
          name="description"
          content="KangaFinance allows for swapping of ERC20 compatible tokens across multiple networks"
        />
      </Head>
      <TokenWarningModal
