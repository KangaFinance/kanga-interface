...
export default function Me() {
  ...
  const { data, error }: SWRResponse<any, Error> = useSWR(
    `https://api.covalenthq.com/v1/${chainId}/address/${account}/stacks/kangafinance/acts/?&key=ckey_cba3674f2ce5450f9d5dd290589&swaps=true&quote-currency=usd`,
    (url) =>
      fetch(url)
        .then((r) => r.json())
        .then((j) => j.data)
  ...
  return (
    <Container id="user-page" className="py-4 space-y-3 md:py-8 lg:py-12" maxWidth="2xl">
      <Head>
        <title>My KANGA | Kanga</title>
        <meta key="description" name="description" content="My KANGA" />
      </Head>
