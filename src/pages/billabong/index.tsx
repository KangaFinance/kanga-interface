...
import { BILLABONG_ADDRESS, ZERO } from '@kangafinance/sdk'
...
import kangaData from '@kangafinance/kanga-data'
import { t } from '@lingui/macro'
import { tryParseAmount } from '../../functions/parse'
import useActiveWeb3React from '../../hooks/useActiveWeb3React'
import { useLingui } from '@lingui/react'
import useSWR from 'swr'
import useBillabong from '../../hooks/useBillabong'
import { useKangaPrice } from '../../services/graph'
...
export default function Stake() {
  ...
  const kangaBalance = useTokenBalance(account ?? undefined, KANGA[ChainId.MAINNET])
  const xKangaBalance = useTokenBalance(account ?? undefined, XKANGA)

  const kangaPrice = useKangaPrice()

  const { enter, leave } = useBillabong()

  const { data } = useSWR(`{billabong(id: "0x8798249c2e607446efb7ad49ec89dd1865ff4272") {ratio, totalSupply}}`, fetcher)

  const xKangaPerKanga = parseFloat(data?.billabong?.ratio)
  ...
  const balance = activeTab === 0 ? kangaBalance : xKangaBalance

  const formattedBalance = balance?.toSignificant(4)

  const parsedAmount = usingBalance ? balance : tryParseAmount(input, balance?.currency)

  const [approvalState, approve] = useApproveCallback(parsedAmount, BILLABONG_ADDRESS[ChainId.MAINNET])
  ...
  useEffect(() => {
    const fetchData = async () => {
      const results = await kangaData.exchange.dayData()
      const apr =
        (((results[1].volumeUSD * 0.05) / data?.billabong?.totalSupply) * 365) / (data?.billabong?.ratio * kangaPrice)

      setApr(apr)
    }
    fetchData()
  }, [data?.billabong?.ratio, data?.billabong?.totalSupply, kangaPrice])

  return (
    <Container id="billabong-page" className="py-4 md:py-8 lg:py-12" maxWidth="full">
      <Head>
        <title key="title">Stake | Kanga</title>
        <meta
          key="description"
          name="description"
          content="Stake KANGA in return for xKANGA, an interest bearing and fungible ERC20 token designed to share revenue generated by all KANGA products."
        />
        <meta key="twitter:url" name="twitter:url" content="https://app.kanga.finance/stake" />
        <meta key="twitter:title" name="twitter:title" content="STAKE KANGA" />
        <meta
          key="twitter:description"
          name="twitter:description"
          content="Stake KANGA in return for xKANGA, an interest bearing and fungible ERC20 token designed to share revenue generated by all KANGA products."
        />
        <meta key="twitter:image" name="twitter:image" content="https://app.kanga.finance/xkanga-sign.png" />
        <meta key="og:title" property="og:title" content="STAKE KANGA" />
        <meta key="og:url" property="og:url" content="https://app.kanga.finance/stake" />
        <meta key="og:image" property="og:image" content="https://app.kanga.finance/xkanga-sign.png" />
        <meta
          key="og:description"
          ...
          <div className="flex flex-col w-full max-w-xl mt-auto mb-2">
            <div className="flex max-w-lg">
              <div className="self-end mb-3 text-lg font-bold md:text-2xl text-high-emphesis md:mb-7">
                {i18n._(t`Maximize yield by staking KANGA for xKANGA`)}
              </div>
              {/* <div className="self-start pl-6 pr-3 mb-1 min-w-max md:hidden">
                                <img src={XKangaSignSmall} alt="xkanga sign" />
                            </div> */}
            </div>
            <div className="max-w-lg pr-3 mb-2 text-sm leading-5 text-gray-500 md:text-base md:mb-4 md:pr-0">
              {i18n._(t`For every swap on the exchange on every chain, 0.05% of the swap fees are distributed as KANGA
                                proportional to your share of the Billabong. When your KANGA is staked into the Billabong, you recieve
                                xKANGA in return for voting rights and a fully composable token that can interact with other protocols.
                                Your xKANGA is continuously compounding, when you unstake you will receive all the originally deposited
                                KANGA and any additional from fees.`)}
            </div>
            {/* <div className="flex">
                            ...
                            <div>
                                <StyledLink className="text-sm text-lg whitespace-nowrap md:text-lg md:leading-5">
                                    Tips for using xKANGA
                                </StyledLink>
                            </div>
                        </div> */}
          </div>
          <div className="hidden px-8 ml-6 md:block w-72">
            <Image src="/xkanga-sign.png" alt="xKANGA sign" width="100%" height="100%" layout="responsive" />
          </div>
        </div>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between w-full h-24 max-w-xl p-4 rounded md:pl-5 md:pr-7 bg-light-yellow bg-opacity-40">
                <div className="flex flex-col">
                  ...
                  <div className="flex">
                    <a
                      href={`https://analytics.kanga.finance/billabong`}
                      target="_blank"
              ...
              <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                ...
                <div className="flex items-center justify-between w-full mt-6">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">
                    {activeTab === 0 ? i18n._(t`Stake KANGA`) : i18n._(t`Unstake`)}
                  </p>
                  <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
                    {`1 xKANGA = ${xKangaPerKanga.toFixed(4)} KANGA`}
                  </div>
                </div>
                ...
                <div className="relative w-full h-0 pointer-events-none bottom-14">
                  ...
                  >
                    <div className="flex space-x-2 ">
                      ...
                      >
                        {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}KANGA`}
                      </p>
                    </div>
              ...
              <div className="flex flex-wrap">
                <div className="flex flex-col flex-grow md:mb-14">
                  ...
                  <div className="flex items-center space-x-4">
                    <Image
                      ...
                      src="/images/tokens/xkanga-square.jpg"
                      alt="xKANGA"
                      width={64}
                      height={64}
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {xKangaBalance ? xKangaBalance.toSignificant(4) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">xKANGA</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex items-center ml-8 space-x-4 md:ml-0">
                    <Image
                      className="max-w-10 md:max-w-16 -ml-1 mr-1 md:mr-2 -mb-1.5 rounded"
                      src="/images/tokens/kanga-square.jpg"
                      alt="KANGA"
                      width={64}
                      height={64}
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {kangaBalance ? kangaBalance.toSignificant(4) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">KANGA</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col w-full mb-4 mt-7 md:mb-0">
                  ...
                  {account && (
                    <a
                      href={`https://analytics.kanga.finance/users/${account}`}
                      target="_blank"
                      rel="noreferrer noopener"
                      className={`
