import { Borrow, Repay } from '../../../features/troop'
import Provider, { useTroopInfo, useTroopPair } from '../../../features/troop/context'
...
function Pair() {
  ...
  const pair = useTroopPair(router.query.pair as string)
  ...
  const onUpdateExchangeRate = useCallback(async () => {
    ...
    const result = await maker.updateExchangeRate().make()
  ...
  return (
    <>
      <Head>
        <title>{i18n._(t`Borrow ${pair?.asset?.symbol}-${pair?.collateral?.symbol}`)} | Kanga</title>
        <meta
          key="description"
          name="description"
          content={`Borrow ${pair?.asset?.symbol} against ${pair?.collateral?.symbol} collateral on Troop by Kanga`}
        />
      </Head>
      <Card
      ...
      right={
        <Card className="h-full bg-dark-900">
          <div className="flex-col space-y-2">
            ...
            <div className="flex justify-between pt-3">
              <div className="text-xl text-high-emphesis">{i18n._(t`Mob`)}</div>
            </div>
            <div className="flex justify-between">
              ...
              <div className="flex flex-row text-lg text-high-emphesis">
                ...
                <QuestionHelper
                  text={i18n._(
                    t`Mob strategies can create yield for your collateral tokens. This token does not yet have a strategy in the Mob.`
                  )}
                />
              </div>
            </div>
            {pair && pair.oracle.name === 'KangaFinance' && (
              <>
                <div className="flex justify-between pt-3">
                  <div className="text-xl text-high-emphesis">{i18n._(t`KLP`)}</div>
                </div>
                {liquidityPair ? (
                  <>
                    <div className="flex justify-between">
