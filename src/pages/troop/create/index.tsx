import { CHAINLINK_ORACLE_ADDRESS, Currency, TROOP_ADDRESS } from '@kangafinance/sdk'
...
import { useMobContract } from '../../../hooks/useContract'
...
function Create() {
  ...
  const handleCreate = async () => {
    try {
      ...
      if (!(chainId in TROOP_ADDRESS)) {
        console.log('No troop address')
        return
      }

      const oracleAddress = CHAINLINK_ORACLE_ADDRESS[chainId]

      const troopData = defaultAbiCoder.encode(
      ...
      const tx = await mobContract?.deploy(chainId && TROOP_ADDRESS[chainId], troopData, true)

      addTransaction(tx, {
        summary: `Add Troop market ${currencies[Field.ASSET].symbol}/${currencies[Field.COLLATERAL].symbol} Chainlink`,
      })
  ...
  return (
    <>
      <Head>
        <title>Create Lending Pair | Troop by Kanga</title>
        <meta key="description" name="description" content="Create Lending Pair on Troop by Kanga" />
      </Head>
      <Card
    ...
    <Layout
      left={
        <Card
          ...
          title={i18n._(t`Create a new Troop Market`)}
          description={i18n._(
