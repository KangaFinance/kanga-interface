...
export default function Vesting() {
  ...
  useEffect(() => {
    const fetchLockup = async () => {
      if (account) {
        fetch('https://raw.githubusercontent.com/kangafinance/kanga-vesting/master/amounts-10959148-12171394.json')
  ...
  if (!pendingTreasurySignature && Number(unclaimedAmount?.toFixed(8)) > 0) {
    vault = 'https://raw.githubusercontent.com/kangafinance/kanga-content/master/images/kanga-vault-reverse.png'
  } else if (!pendingTreasurySignature && Number(unclaimedAmount?.toFixed(8)) <= 0) {
    vault = 'https://raw.githubusercontent.com/kangafinance/kanga-content/master/images/vesting-safe-off.png'
  } else if (pendingTreasurySignature) {
    vault = 'https://raw.githubusercontent.com/kangafinance/kanga-content/master/images/vesting-safe-closed.png'
  }

  return (
    <Container id="vesting-page" className="py-4 md:py-8 lg:py-12 m-auto w-full max-w-[900px]">
      <Head>
        <title>Vesting | Kanga</title>
        <meta key="description" name="description" content="KangaFinance vesting..." />
      </Head>
      <div className="flex px-0 sm:px-4 md:flex-row md:space-x-10 lg:space-x-20 md:px-10">
        <div className="hidden space-y-10 md:block">
          <Image src={vault} loader={undefined} width={340} height={300} alt="" />
          <div className="relative w-full p-4 overflow-hidden rounded bg-dark-900">
            ...
            >
              <>
                ...
                <ExternalLink href="https://snapshot.org/#/kanga/proposal/QmPwBGy98NARoEcUfuWPgzMdJdiaZub1gVic67DcSs6NZQ">
                  SIMP3
                </ExternalLink>
                .
                <br />
                ...
                <ExternalLink href="https://forum.kangafinanceclassic.org/t/simp-3-vesting-and-the-future-of-kangafinance/1794">
                ...
                <ExternalLink href="https://github.com/kangafinance/kanga-vesting">Github</ExternalLink>
              </>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-3 max-w-[400px]">
          <div className="relative w-full overflow-hidden rounded bg-dark-900">
            <div className="flex flex-col gap-3 p-4">
              <div className="flex flex-row justify-between">
                <div className="font-bold text-white">{i18n._(t`Your Claimable KANGA this Week`)}</div>
                <QuestionHelper text="Your Vested KANGA will be released each week for the next 6 months. The amount released each week is determined by your historical farming rewards. You do not need to harvest each week as unclaimed amounts from each week will continue to accrue onto the next." />
              </div>
              ...
              <div className="flex flex-col items-baseline">
                ...
                {account ? (
                  <div className="text-sm text-secondary">
                    {totalLocked ? (
                      i18n._(t`Historical Total Locked: ${formatNumber(totalLocked)} KANGA`)
                    ) : (
              ...
              >
                {pendingTreasurySignature ? (
                  ...
                  <> {claimConfirmed ? i18n._(t`Claimed`) : i18n._(t`Claim KANGA`)}</>
                )}
          ...
          <div className="relative w-full overflow-hidden rounded bg-dark-900">
            <div className="flex flex-col gap-3 p-4">
              <div className="font-bold text-white">{i18n._(t`Things you can do with your KANGA`)}</div>
              <div className="p-4 rounded bg-dark-800">
                <Link href="/stake">
                  <a className="flex items-center justify-between gap-3">
                    <div className="flex flex-col gap-1">
                      <div className="font-bold text-white">{i18n._(t`Stake KANGA for xKANGA`)}</div>
                      <div className="text-sm text-secondary">
                        {t`Gain governance rights with xKANGA and earn 5% APR (0.05% of
                ...
                <Link href={`/saave`}>
                  <a className="flex items-center justify-between gap-3">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="text-sm text-secondary">
                        {t`Stake into xKANGA add collateral as axKANGA on Aave all in
                                                            one click`}
                      </div>
                    </div>
                    <div className="min-w-[32px]">
              ...
              <div className="p-4 rounded bg-dark-800">
                <div className="flex flex-col gap-1">
                  <div className="font-bold text-white">{i18n._(t`Deposit KANGA into Mob`)}</div>
                  <div className="text-sm text-secondary">
