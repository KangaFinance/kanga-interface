import { useBlock, useKangaPairs } from '../../../services/graph'
...
export default function Pairs() {
  ...
  const pairs = useKangaPairs({ chainId })
  const pairs1d = useKangaPairs({ block: block1d, shouldFetch: !!block1d, chainId })
  const pairs2d = useKangaPairs({ block: block2d, shouldFetch: !!block2d && type !== 'all', chainId }) // No need to fetch if we don't need the data
  const pairs1w = useKangaPairs({ block: block1w, shouldFetch: !!block1w, chainId })
  const pairs2w = useKangaPairs({ block: block2w, shouldFetch: !!block2w && type !== 'all', chainId })
