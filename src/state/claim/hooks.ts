import { ChainId, Currency, CurrencyAmount, JSBI, Token } from '@kangafinance/sdk'
...
export function useUserUnclaimedAmount(account: string | null | undefined): CurrencyAmount<Currency> | undefined {
  ...
  const kanga = chainId ? KANGA[chainId] : undefined
  ...
  if (!kanga) return undefined
  if (!canClaim || !userClaimData) {
    return CurrencyAmount.fromRawAmount(kanga, JSBI.BigInt(0))
  }
  return CurrencyAmount.fromRawAmount(kanga, JSBI.BigInt(userClaimData.amount))
}
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unClaimedAmount?.toSignificant(4)} KANGA`,
