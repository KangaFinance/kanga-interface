...
import { ChainId, Currency, CurrencyAmount, JSBI, NATIVE, Percent, Price, WNATIVE } from '@kangafinance/sdk'
...
import { useMobBalances } from '../mob/hooks'
...
export function useDerivedLimitOrderInfo(): {
  ...
  mobBalances: { [field in Field]?: CurrencyAmount<Currency> }
  inputError?: string
  ...
  const mobTempBalances = useMobBalances()
  const balance = useMemo(
    () => mobTempBalances?.find((el) => el.address === inputCurrency?.wrapped.address),
    [mobTempBalances, inputCurrency?.wrapped.address]
  )
  ...
  const mobBalances = {
    [Field.INPUT]: inputCurrency
      ? CurrencyAmount.fromRawAmount(inputCurrency, balance?.mobBalance ? balance.mobBalance : 0)
      : undefined,
    [Field.OUTPUT]: outputCurrency
      ? CurrencyAmount.fromRawAmount(outputCurrency, balance?.mobBalance ? balance.mobBalance : 0)
      : undefined,
  }
  ...
  const [balanceIn, amountIn] = [
    fromMobBalance ? mobBalances[Field.INPUT] : walletBalances[Field.INPUT],
...
export function queryParametersToSwapState(chainId: ChainId, parsedQs: ParsedQs) {
  ...
  return {
    ...
    fromMobBalance: parseBooleanFieldParameter(parsedQs.fromMob),
