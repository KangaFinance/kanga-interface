...
Currently there is two types of oracles which the kanga interface supports, Chainlink & KangaFinance TWAP.
...
Each price feed has a pair token0/token1, KANGA/ETH or KANGA/USD for example, decimals, and proxy. We use this information to construct mappings of the price feed to their respectful pair tokens.
...
If a newly added token is not in the @kangafinance/default-token-list, it should be added.
